...
- UniApp 移动端通过 `uni.navigateTo` 打开页面时，目标页面必须在 `pages.json` 的 `pages` 或 `subPackages` 中注册；登录页协议、隐私政策这类内联可点击文案应使用 `<text @click.stop="...">`，避免 H5 标签或事件冒泡导致真机点击无效。
...
**Fix**: 对协议、隐私政策等静态展示页，新增 `src/pages/<module>/<page>.vue` 后同步注册 `src/pages.json`；登录页内联链接使用 UniApp 原生 `<text>` 承载点击事件，并加 `.stop` 阻止事件继续冒泡。注册缺失会导致 `uni.navigateTo({ url })` 在移动端直接失败，表现为“点击打不开”。
